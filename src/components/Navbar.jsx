...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-[100] transition-all duration-500 ${scrolled ? 'bg-white/80 backdrop-blur-md py-2 shadow-sm' : 'bg-transparent py-4'}`}>
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {links.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="relative font-medium transition-colors duration-300 group text-[#3A2D28] hover:text-[#682E36] text-sm tracking-wide"
              >
                {link.name}
                <span className="absolute left-0 bottom-[-4px] w-0 h-[1.5px] bg-[#3A2D28] transition-all duration-300 group-hover:w-full"></span>
              </Link>
            ))}
            ...
            <div className="relative">
              <button
                ...
                className="px-6 py-2 rounded-full font-medium transition-all duration-300 border flex items-center gap-2 bg-[#3A2D28] text-[#FAF6F0] hover:bg-[#2A1D18] text-sm tracking-wide"
              >
                Contact
              </button>
              ...
              <AnimatePresence>
                {contactOpen && (
                  ...
                  >
                    <a href="tel:+917396756920" className="flex items-center gap-3 px-5 py-3 text-sm text-[#3A2D28] hover:bg-cream-100 transition-colors" onClick={() => setContactOpen(false)}>
                      <FiPhoneCall className="text-gold-600" /> Call Us
                    </a>
                    <a href="https://wa.me/917396756920" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-5 py-3 text-sm text-[#3A2D28] hover:bg-cream-100 transition-colors" onClick={() => setContactOpen(false)}>
                      <FiMessageCircle className="text-gold-600" /> WhatsApp
                    </a>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <div className="flex items-center md:hidden z-[110]">
            <button
              ...
              className="p-2 rounded-full focus:outline-none transition-colors text-[#3A2D28] hover:bg-black/5"
            >
